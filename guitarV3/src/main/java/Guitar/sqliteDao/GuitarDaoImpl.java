...
public class GuitarDaoImpl implements GuitarDao{
	...
	public boolean add(Guitar guitar) {
		// TODO Auto-generated method stub
		return DbUtil.executeUpdate("insert into guitar values(?,?,?,?,?,?,?,?)", new Object[]{guitar.getSerialNumber(),guitar.getPrice(),guitar.getSpec().getBuilder(),guitar.getSpec().getModel(),guitar.getSpec().getType(),guitar.getSpec().getNumStrings(),guitar.getSpec().getBackWood(),guitar.getSpec().getTopWood()});		
	}
	public boolean delete(Guitar guitar) {
		// TODO Auto-generated method stub
		return DbUtil.executeUpdate("delete from guitar where serialNumber=?", new Object[]{guitar.getSerialNumber()});		
	}
}